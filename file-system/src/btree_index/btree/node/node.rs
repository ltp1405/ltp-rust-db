        ...
        match self.node_type() {
            NodeType::Leaf => {
                ...
                for i in 0..self.num_cells() {
                    let cell = unsafe {
                        Cell::leaf_at(page.as_ptr() as *const u8 + self.cell_pointer(i) as usize)
                    };
                    cells.push(cell);
                }
            ...
            NodeType::Interior => {
                ...
                for i in 0..self.num_cells() {
                    let cell = unsafe {
                        Cell::interior_at(
                            page.as_ptr() as *const u8 + self.cell_pointer(i) as usize,
                        )
                    };
    ...
    Node<'a, BLOCKSIZE, CAPACITY, MEMORY_CAPACITY>
{
    pub fn new(
        ...
        let new_page = disk_manager.allocate_page();
        let mut node = Node {
    ...
    pub fn node_type(&self) -> NodeType {
        let page = self.page();
        let node_type = unsafe { NodeHeaderReader::new(page).node_type() };
    ...
    fn right_child(&self) -> NodePointer {
        ...
        let page = self.page();
        let right_child = unsafe { NodeHeaderReader::new(page).right_child() };
    }

    pub fn set_right_child(&self, child: NodePointer) {
        assert_eq!(self.node_type(), NodeType::Interior);
        let page = self.page();
        unsafe { NodeHeaderWriter::new(page).set_right_child(child) };
    }
    ...
    fn cell_content_start(&self) -> u32 {
        let page = self.page();
        unsafe { NodeHeaderReader::new(&page).cell_content_start() }
    }

    fn set_cell_content_start(&mut self, val: u32) {
        let page = self.page();
        unsafe { NodeHeaderWriter::new(&page).set_cell_content_start(val) }
    }

    fn set_cell_pointer(&mut self, cell_num: u32, val: u32) {
        let page = self.page();
        unsafe { NodeHeaderWriter::new(&page).set_cell_pointer(cell_num, val) }
    }
    ...
    fn cell_pointer(&self, cell_num: u32) -> CellPointer {
        let page = self.page();
        unsafe { NodeHeaderReader::new(&page).cell_pointer(cell_num) }
    }
...
mod node {
    ...
    use crate::btree_index::btree::node::{node::InsertResult, node_header::NodeType};
